...
def demonstrate_bond_model():
    ...
    fluid_velocity = np.array([[0.5 * np.sin(t), 0.0, 0.0]
                              for t in time_steps])
...
def demonstrate_validation():
    ...
    simulation_results = {
        ...
        'erosion_stats': np.random.normal(0.5, 0.1, 100),
        'fluid_data': {
    ...
    # 101 points for pressure/velocity fields
    sim_time_fields = np.linspace(0, 0.5, 101)

    # Interpolate erosion rates
    erosion_interp = interp1d(
        sim_time_erosion, simulation_results['erosion_stats'])
    simulation_results['erosion_stats'] = erosion_interp(exp_data['time'])

    # Interpolate pressure field
    pressure_interp = interp1d(sim_time_fields, np.mean(
        simulation_results['fluid_data']['pressure_field'], axis=(1, 2)))
    simulation_results['fluid_data']['pressure_field'] = pressure_interp(
        exp_data['time'])

    # Interpolate velocity field
    velocity_interp = interp1d(sim_time_fields, np.mean(
        simulation_results['fluid_data']['velocity_field'], axis=(1, 2, 3)))
    simulation_results['fluid_data']['velocity_field'] = velocity_interp(
        exp_data['time'])
    ...
    print(
        f"Simulated erosion rates (first 10): {simulation_results['erosion_stats'][:10]}")
