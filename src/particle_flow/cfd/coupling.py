...
class CFDDEMCoupling:
    def __init__(self, config: Dict):
        ...
        self.interpolation_method = config['coupling'].get(
        ...
        logger.info(
            "CFD-DEM coupling framework initialized with geotechnical parameters")
    ...
    def _calibrate_critical_shear_stress(self) -> float:
        ...
        water_factor = 1.0 - 0.3 * \
            (self.geotech_params['water_content'] / 20.0)
    ...
    def _calibrate_erosion_rate(self) -> float:
        ...
        base_rate = self.config['erosion'].get(
            'erosion_rate_coefficient', 1e-6)

        # Adjust based on permeability
        # Normalize to 1e-6 m/s
        perm_factor = self.geotech_params['permeability'] / 1e-6

        # Adjust based on porosity
        # Normalize to 0.3
        porosity_factor = self.geotech_params['porosity'] / 0.3

        # Adjust based on void ratio
        # Normalize to 0.6
        void_ratio_factor = self.geotech_params['void_ratio'] / 0.6
    ...
    def _linear_interpolation(self, position: np.ndarray) -> np.ndarray:
        ...
        if not (0 <= position[0] <= 1 and 0 <= position[1] <= 1 and 0 <= position[2] <= 1):
            raise ValueError(
                f"Position {position} is outside the domain [0,1]^3")
    ...
    def _compute_erosion_force(self, fluid_velocity: np.ndarray,
        ...
        shear_stress = self._compute_fluid_shear_stress(
        ...
        force_magnitude = self.erosion_rate_coefficient * \
            shear_stress * np.pi * particle_radius**2
    ...
    def _compute_fluid_shear_stress(self, fluid_velocity: np.ndarray,
        ...
        shear_stress = 0.5 * \
            self.config['cfd']['fluid_density'] * \
            np.linalg.norm(fluid_velocity)**2 * Cd
    ...
    def step(self):
        ...
        fluid_forces = self.compute_fluid_forces(
            ...
            [self.config['dem']['particle_radius']] *
            len(particle_data['positions'])
        )
        ...
        if self.coarse_model:
            ...
            bond_forces = self.coarse_model.compute_coarse_bond_forces(
                self.cfd_solver.get_fluid_data(
                )['velocity_field'].mean(axis=(0, 1, 2)),
    ...
    def get_erosion_statistics(self) -> Dict:
        ...
        erosion_rate = total_eroded / (len(self.eroded_particles) *
                                       self.config['simulation']['time_step'])
        average_force = np.mean([p['force']
                                for p in self.eroded_particles], axis=0)
